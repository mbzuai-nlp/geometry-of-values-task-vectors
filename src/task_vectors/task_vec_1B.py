...
size = "1B"
tasks = ["AB", "BC", "CA"]  # Updated to only include these two tasks
...
for lang in langs:
    ...
    for task in tasks:
        ...
        # determine which instruction models to use
        if task == "AB":
            instruction_models_list = ["BC_CB", "CA_AC"]  # use CA+AC for AB
        elif task == "BC":
            instruction_models_list = ["CA_AC", "AB_BA"]  # use CA+AC for BC
        elif task == "CA":
        ...
        for instruction_models in instruction_models_list:
            ...
            with torch.no_grad():
                ...
                alpha = torch.clamp(
                    vec_pref.dot(vec_instr) / instr_norm_sq,
                    min = -1 / alpha_floor,
                    max =  1 / alpha_floor
                )
            ...
            for gamma_instr in fine_gamma_instrs:
                for gamma_pref in fine_gamma_prefs:
                    ...
                    if np.isclose(gamma_instr, best_gamma_instr) and np.isclose(gamma_pref, best_gamma_pref):
                        continue
                    accuracy = evaluate_gammas(gamma_instr, gamma_pref, base_model, tokenizer, delta_instr, pref_vector, task, instruction_models, lang)
            ...
            test_accuracy = evaluate_gammas(best_gamma_instr, best_gamma_pref, base_model, tokenizer, delta_instr, pref_vector, task, instruction_models, lang, split='test')
