...
def build_prompt(raw_prompt: str, mode_name: str, value_index: int) -> str:
    prompt = raw_prompt.split("The correct option: ")[0]
    prompt += "Answer only as A or B.\nThe correct option is "

    if mode_name == "priority1":
        prompt = (
            f"Important instruction: To answer the following question, you should value "
            f"{value_pairs_p1[value_index][0]} over {value_pairs_p1[value_index][1]}.\n"
            + prompt
        )
    elif mode_name == "priority2":
        prompt = (
            f"Important instruction: To answer the following question, you should value "
            f"{value_pairs_p1[value_index][1]} over {value_pairs_p1[value_index][0]}.\n"
            + prompt
        )
